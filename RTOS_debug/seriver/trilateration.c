...
/*
3边定位算法实现
best_solution		最终结果输出
use4thAnchor		模式,是否使用4基站
anchorArray			基站坐标数组首地址
distanceArray		距离(mm)
*/
...
SemaphoreHandle_t report_useflag;
//最终标签坐标
vec3d report;
//4基站距离
int Range_deca[4];	
...
void tril_init(void)
{
	...
	anchorArray[0].x =	A0_X; 
	anchorArray[0].y = 	A0_Y;
	anchorArray[0].z = 	A0_Z;
	...
	UWB_init();
	report_useflag = xSemaphoreCreateMutex();
}
...
int tril_read(void)
{
	...
	for(i = 0;i < 4;i++)
	{
		anchorArray[i].x = Cell_Site[i].x;
		anchorArray[i].y = Cell_Site[i].y;
		anchorArray[i].z = Cell_Site[i].z;
	}
	for (j = 0; j < 4; j++)
	{
		for (i = 0; i < 9; i++)
		{
			temp[j][i] = UWB_data[j * 9 + i + 6];
		}
		temp[j][8] = '\0';
		Range_deca[j] = (int)strtoul(temp[j],NULL,16);
		if(Range_deca[j] != -1)
			num_flag++;
	}
	...
	return GetLocation(&report, 1, &anchorArray[0], &Range_deca[0]);
}
