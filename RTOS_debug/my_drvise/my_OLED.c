...
{
	R_IIC_MASTER_Open(g_i2c_1.p_ctrl,g_i2c_1.p_cfg);
}
...
/**
  ...
  * @retval 无
  */
void OLED_I2C_SendByte(uint8_t *const p_src,uint32_t size)
{
	
	R_IIC_MASTER_Write(g_i2c_1.p_ctrl,p_src,size,false);
	I2C2WaitTxCplt();
}
...
void OLED_ShowFT(uint8_t Line, uint8_t Column, FontT Char)
{      	
	uint8_t i;
		OLED_SetCursor((Line - 1) * 2, (Column - 1) * 8);		//设置光标位置在上半部分
		for (i = 0; i < 8; i++)
		{
			OLED_WriteData(my_F8x16[Char][i]);			//显示上半部分内容
		}
		OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部分
		for (i = 0; i < 8; i++)
		{
			OLED_WriteData(my_F8x16[Char][i + 8]);		//显示下半部分内容
		}
